...
from ultralytics.nn.foundation.npy_cache import NPY_SCHEMA_VERSION, validate_npy_evidence
...
def identity(args):
    return {
        ...
        "contract": cache_contract(args.repo),
...
def _worker(args, own, split, selected_batches, part_dir):
    ...
    writer = FeatureCacheWriter(
        ...
        target_shard_bytes=SHARD_BYTES,
        shard_prefix=f"{split}-r{args.rank:02d}",
    )
    ...
    teacher = DINOv3Teacher(
        model_id=own["contract"]["model_id"],
        ...
        if not all(writer.is_cached(r["sample_id"], r["image_sha256"]) for r in batch_rows):
            ...
            for i, row in enumerate(batch_rows):
                ...
                writer.add(
                    ...
                    features={n: dense[n][i] for n in FEATURE_NAMES},
                )
        ...
        if number % 8 == 0:
            write_json(
                report_path,
                {
                    ...
                    "split": split,
                    "processed": done,
                    "total": sum(map(len, selected_batches)),
    ...
    for batch_index in sorted({0, len(selected_batches) - 1}):
        ...
        dense = teacher.encode(
            torch.stack([load_image(args.data / r["image_path"], letterbox) for r in batch_rows])
        ).dense
        for i, row in enumerate(batch_rows):
...
def finalize(args, split, own):
    ...
    for rank in range(own["world_size"]):
        ...
        expected_contexts = [
            [r["sample_id"] for r in b] for b in batches(expected_rows, rank, own["world_size"], args.batch)
        ]
        if (
            report.get("status") != "PASSED"
            or report.get("identity") != own
            or report.get("rank") != rank
            or report.get("split") != name
...
def execute(args):
        ...
        try:
            ...
            for child in children:
                try:
                    ...
                    child.wait()
        write_json(
            args.output / "status.json", {"status": "RUNNING", "phase": f"verify-convert-{split}", "pid": os.getpid()}
        )
        results[split] = {
            "safetensors": finalize(args, split, own),
            "npy": convert_preserving_source(args.output / "safetensors" / f"visdrone-{split}", args.output / "npy"),
        }
    ...
    for split in ("train", "val"):
        name = f"visdrone-{split}"
        results[split]["readiness"] = validate_npy_evidence(
            npy_root / name, npy_root / "summary.json", name, COUNTS[split]
        )
    write_json(
        args.output / "cache-summary.json",
        {
            "status": "PASSED",
            "identity": own,
            "splits": results,
            "elapsed_seconds": time.monotonic() - started,
            "test_dev_extracted": False,
            "training_started": False,
            "e2_overall_complete": False,
        },
    )
