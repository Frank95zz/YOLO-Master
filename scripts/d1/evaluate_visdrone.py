...
def export_predictions(predictions, image_ids, output, *, conf=0.001, max_det=500):
    ...
    result = {"schema_version": "d1-visdrone-export-v1", "image_count": len(image_ids), "conf": conf,
              "max_det": max_det, "coordinates": "original-image xywh", "category_mapping": "0..9 -> 1..10",
              ...
              "toolkit_commit": TOOLKIT_COMMIT, "files_sha256": files}
...
def make_fixture(source, output):
    ...
    for image in images:
        ...
        for row in csv.reader(annotation.read_text().splitlines()):
            ...
            if values[4] == 1 and 1 <= values[5] <= 10 and values[2] > 0 and values[3] > 0:
                predictions.append({'image_id': image.stem, 'category_id': int(values[5])-1,
                                    'score': .9, 'bbox': values[:4]})
    ...
    result = {'fixture_not_model_accuracy': True, 'source_split': 'val', 'image_ids':[p.stem for p in images],
              'files_sha256':files, 'export':exported}
