...
def convert_annotation(text, width, height):
    ...
    for number, row in enumerate(csv.reader(io.StringIO(text)), 1):
        ...
        if valid and (x2 <= x1 or y2 <= y1):
            ...
            label = [int(category) - 1, (x1 + x2) / (2 * width), (y1 + y2) / (2 * height),
        ...
        sidecar.append({"line": number, "original": values, "clipped_xywh": clipped,
                        "clipped": changed, "disposition": reason})
...
def prepare(source, output, *, verify=False, counts=None):
    ...
    for split, count in counts.items():
        ...
        for image in images:
            ...
            inventory.append({"sample_id": sid, "split": f"visdrone-{split}", "image_path": rel,
                              "original_image": image.relative_to(source).as_posix(),
                              "original_annotation": ann.relative_to(source).as_posix(),
                              "image_sha256": image_sha, "annotation_sha256": digest(annotation),
                              "label_path": label_rel, "label_sha256": digest(label),
                              "sidecar_path": side_rel, "sidecar_sha256": digest(side_bytes),
                              "width": width, "height": height})
            paths.append(rel)
        previous_hashes.update(split_hashes)
        listing = ("\n".join(paths) + "\n").encode()
        immutable(output / f"{split}.txt", listing, verify)
        reports[split] = {"images": count, "paths_sha256": digest(listing), "counts": dict(totals),
    ...
    import yaml
    cfg = {"path": str(output), "train": "images/visdrone-train", "val": "images/visdrone-val",
           "test": "images/visdrone-test-dev", "nc": 10, "names": list(NAMES)}
    ...
    result = {"schema_version": SCHEMA, "dataset": "VisDrone2019-DET", "splits": reports,
              "samples_sha256": digest(inventory_bytes), "sample_count": len(inventory),
              "class_names": list(NAMES), "originals_preserved": True, "split_ids_disjoint": True,
              "split_image_hashes_disjoint": True, "source_download_receipt": source_receipt,
              "source_download_receipt_sha256": file_sha(original_manifest) if source_receipt else None,
              "training_ignore_background_mask": False, "test_dev_use": "final evaluation only"}
