...
def extract_zip(path: Path, destination: Path) -> None:
    ...
    directories = {
        destination / (Path(member.filename) if member.is_dir() else Path(member.filename).parent) for member in members
    }
    ...
    pending_members = [
        member for member in members if not member.is_dir() and existing_sizes.get(member.filename) != member.file_size
    ]
    chunk_size = max(1, math.ceil(len(pending_members) / EXTRACT_WORKERS))
    chunks = [pending_members[index : index + chunk_size] for index in range(0, len(pending_members), chunk_size)]
...
def generate_manifests(workspace: Path, repo: Path, load_model: bool) -> None:
    coco_root = workspace / "datasets" / "coco"
    teacher_root = workspace / "weights" / "teachers" / "dinov3-vits16-pretrain-lvd1689m"
    output_root = workspace / "manifests"
    ...
    materialize_splits(coco_root, repo)
