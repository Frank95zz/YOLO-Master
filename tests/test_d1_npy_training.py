...
def test_launcher_requires_approval_and_a_safe_run_name():
    from scripts.d1.launch_wp8_npy import Pipeline

    with pytest.raises(ValueError, match="approval"):
        Pipeline(SimpleNamespace(approved=False))
    with pytest.raises(ValueError, match="run-id"):
        Pipeline(SimpleNamespace(approved=True, run_id="../old-experiment"))


def test_code_fingerprint_includes_new_and_modified_runtime_sources(tmp_path):
    from scripts.d1.run_wp8_train import code_fingerprint

    root = tmp_path / "ultralytics"
    root.mkdir()
    module = root / "new_reader.py"
    module.write_text("VALUE = 1\n")
    original = code_fingerprint(tmp_path)
    assert code_fingerprint(tmp_path) == original
