...
def test_install_occurs_after_resume_setup(implementation, monkeypatch):
    model = p5.construct_model("DW")
    existing = ModelEMA(model, updates=27)
    trainer = object.__new__(p5.P5FrozenTrainer)
    trainer.e1 = {"ema_implementation": implementation, "identity": {"ema_implementation": implementation}}
    calls = []

    def setup(self):
        calls.append("resume_setup")
        self.model, self.ema = model, existing

    monkeypatch.setattr(E1FrozenTrainer, "_setup_train", setup)
    trainer._setup_train()
    assert calls == ["resume_setup"]
    assert trainer.ema.ema is existing.ema and trainer.ema.updates == 27
    assert type(trainer.ema) is (D1ModelEMA if implementation == "foreach-v1" else ModelEMA)
    trainer.e1["ema_implementation"] = "typo"
    with pytest.raises(ValueError):
        trainer._setup_train()
    trainer.e1["ema_implementation"] = "foreach-v1"
    trainer.e1["identity"]["ema_implementation"] = "scalar-v1"
    with pytest.raises(ValueError, match="identity"):
        trainer._setup_train()


def test_identity_and_cli_are_fail_closed():
    matrix = {
        "workspace": "/external",
        "identity": {},
        "parameters": {"BASE": 1},
        "data_receipt_sha256": "data",
        "input_hashes": {"initial-BASE.pt": "a", "model-BASE.yaml": "b"},
    }
    old = p5.spec_for(matrix, "BASE")
    assert old["ema_implementation"] == "scalar-v1" and "ema_implementation" not in old["identity"]
    matrix["ema_implementation"] = "foreach-v1"
    new = p5.spec_for(matrix, "BASE")
    assert new["identity"]["ema_implementation"] == "foreach-v1" and new["identity"] != old["identity"]
    with pytest.raises(SystemExit):
        p5.main(["train", "--approved", "--ema-implementation", "foreach-v1"])
    with pytest.raises(SystemExit):
        p5.main(["evaluate", "--ema-implementation", "foreach-v1"])


@pytest.mark.parametrize("policy", ("legacy", "epoch-boundary-v1"))
@pytest.mark.parametrize("resume", (False, True))
def test_registered_resume_temperature_boundary(policy, resume, monkeypatch):
    from ultralytics.engine.extensions.mixture import MixtureRuntimeController

    model = p5.construct_model("BASE")
    trainer = object.__new__(p5.P5FrozenTrainer)
    trainer.e1 = {"resume_temperature_policy": policy, "identity": {"resume_temperature_policy": policy}}
    trainer.resume = resume
    trainer.start_epoch = 1 if resume else 0
    trainer.args = SimpleNamespace(moa_mot_temperature_factor=0.97, moa_mot_min_temperature=0.3)
    trainer.model = model
    trainer.ema = ModelEMA(model)
    trainer.mixture_controller = MixtureRuntimeController(trainer)
    monkeypatch.setattr(E1FrozenTrainer, "_setup_train", lambda self: None)
    trainer._setup_train()
    trainer.mixture_controller.begin_epoch(trainer.start_epoch)
    expected = torch.tensor(0.97 if resume and policy == "epoch-boundary-v1" else 1.0)
    for mixture in model.mixtures.values():
        assert torch.equal(mixture.router.temperature, expected)
