...
在干净代码提交 f5bf7bc56d128e02d3485fe8df1e15301bee7556 上，主回归为419通过、56跳过；共享 Foundation/引擎补充回归为121通过，单列排除2项已在相同上游提交复现的既有失败。合计540通过，不宣称整个仓库测试全部通过。48个选定 Python 文件的 Ruff、格式与编译检查，以及文档相对链接、git diff --check 均通过；codespell 未安装，未执行。

验收另用合成数据完成一轮训练、保存、严格重载及 COCO/VisDrone 独立导出。真实权重/CUDA 测试需显式启用；本轮不下载模型、不运行真实六卡训练。测试清单、日志摘要校验值、模型配置 SHA256 和核心文件语法树一致性记录见 [pr-verification.json](manifests/pr-verification.json)。
