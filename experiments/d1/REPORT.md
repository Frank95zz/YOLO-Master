...
验收范围为离线 D1/Foundation、LatentMixture、aux、恢复、checkpoint、EMA 和配置回归，另用合成数据完成一轮训练、保存、严格重载及两种数据集导出。真实权重/CUDA 测试需显式启用；本轮不下载模型、不运行真实六卡训练。最终测试数量和代码身份记录于随代码提交后生成的小型验证证据。
