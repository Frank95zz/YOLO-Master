...
本项目已完成 P0 冻结特征检测闭环和 P2 latent aux 消融。P1 已完成两数据集的精度/显存/时长对照及 VisDrone 三 seed 汇总；11/12 组正式结果已归档，COCO Scratch seed 2 完成后补齐 COCO 三 seed 统计。
...
| P1：精度 / 显存 / 时长三维对照 | **两数据集对照及 VisDrone 三 seed 汇总已完成** | 两数据集、两架构、三个 seed 的统一合同与队列已实现、验证并启动 | 按统一口径报告 AP、显存、GPU-hours、精度保留率及成本降低率，对照至少 50% 降本目标 |
...
**新增 COCO 结果：** seed 1 两组均完成固定第 50 轮标准 bbox 评测和严格重载，BN64 / Scratch 的 AP 为 **29.302 / 26.976**，保留率 **108.62%**，训练 GPU-hours 降低 **67.68%**。周期最佳单列为 BN64 第 40 轮 AP 29.389、Scratch 第 20 轮 AP 31.596。BN64 seed 2 的固定末轮 AP 为 **28.517**，周期最佳为第 35 轮 AP 28.704。COCO Scratch seed 2 是剩余的一组训练；完成后按同一末轮口径汇总 COCO 三 seed。

本次新增的五组记录使用执行提交 ee23d3edf04b3741e1a6441c57a9cc2ff4d2516c、配方 SHA256 c4baac70164f7b91e93d1d164864c35038c3fddfcea2d021a027bf176c3abee0；预算、数据、batch 和评测规则沿用本节合同。各组的种子、数据列表、模型配置、checkpoint、预测与评分 SHA256，以及六 rank 更新和显存，见复现包 results/ 对应实验记录。已完成 11/12 组固定末轮训练与官方评分，结果表与机器可读文件同步更新。
