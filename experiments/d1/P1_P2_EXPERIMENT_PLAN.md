...
本次根据 E1 阶段结果补充第 3.1、6.2-6.4、8.3、11.4 和 12.2 节。保留 E1 已登记的固定轮数、评测和可训练参数匹配合同，以及后续多 seed 和成本判据；不修改正在执行的 S 组合同，也不以更新文档授权启动后续训练。
...
| S | 同可训练参数量 Scratch / 不适用 | 待第 50 轮完成 | 待完整窗口汇总 | 待定 |
...
AUX_STAR 与最终冻结架构锁定、正式基线通过第 9 节确认后，再进入两数据集配对训练，不按各数据集的结果临时分别挑选最佳 aux。
