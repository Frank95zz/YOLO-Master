...
实际实现、验收、运行入口及结果边界见 [E0/E1 执行记录](https://github.com/Frank95zz/YOLO-Master/blob/373b8a4a563bd885c13b78652a75ae4dee7edfdd/experiments/d1/E0_E1.md)和 [E2 执行记录](E2.md)。这不代表已经达到 P1/P2 收益目标。
...
**2026-09-11 当前决定：用户确认完整系统总参数匹配，基线采用 YOLO26-L width0.9375。** 该决定取代上面的延后选型状态，但不改变 E1 历史身份；完整配对训练仍受第 9 节门禁约束。详见 [总参数基线说明](SCRATCH_TOTAL_PARAMETER_BASELINE.md)。
...
| P1：课题页要求同参数量，细化任务书要求同预算 | 2026-09-11 用户确定按完整系统总参数匹配，采用 YOLO26-L width0.9375；当前 E1 仍只匹配可训练参数；配对数据、预算、评测和硬件另行保持一致 | 参数口径确认记录、完整系统/冻结/可训练参数审计、正式基线配置、实际更新次数和时间轴 |
...
- [后续实验准备](https://github.com/Frank95zz/YOLO-Master/blob/373b8a4a563bd885c13b78652a75ae4dee7edfdd/scripts/d1/inspect_wp8_followup.py)
- [当前 A/B/C 准备配置](https://github.com/Frank95zz/YOLO-Master/blob/373b8a4a563bd885c13b78652a75ae4dee7edfdd/ultralytics/cfg/experiments/d1/wp8-followup.yaml)
...
当前可运行命令以 [E0/E1 执行记录](https://github.com/Frank95zz/YOLO-Master/blob/373b8a4a563bd885c13b78652a75ae4dee7edfdd/experiments/d1/E0_E1.md)和 [E2 执行记录](E2.md)为准，不应把后续接口设想当成已经实现。
...
4. **总参数口径和 Scratch 架构已于 2026-09-11 确定，满足其余门禁后再进入 E4。** AUX_STAR、完整训练合同和预算尚需锁定；形成两数据集、三 seed 的正式配对结果前，须通过第 9 节确认。如果触发第 6 节的 80% 暂停线，仍先执行第 8 节的解除条件，不自动展开 COCO 长训练。
...
E1 Scratch 是小型 CNN；本次 E4 Scratch 是约 2300 万参数的 YOLO26 卷积模型，二者都不是从零训练 DINOv3。不能用“省去了 DINOv3 反向”直接推导其相对 Scratch 必然节省 50%。
...
#### 4.3.3 正式基线的总参数决定（2026-09-11）

用户已确认采用完整系统总参数。冻结目标为 ViT-S + BN64；Scratch 采用标准 YOLO26-L 拓扑，depth=1.0、width=0.9375、max_channels=512，随机初始化。两个数据集共用骨干和 Neck，仅切换 nc。
两个数据集参数偏差均小于 1%：VisDrone +0.4165%，COCO +0.5746%。这是本项目的工程容差，不冒充官方定义。实测参数、候选选择和测试见 [总参数基线说明](SCRATCH_TOTAL_PARAMETER_BASELINE.md)。
本次没有增加同架构随机 DINO 对照，也没有开始正式训练；若更改 Teacher、BN64 通道或检测头，需要重新审计匹配关系。
...
| 指标 | 定义与单位 | 对照时的约束 |
| --- | --- | --- |
| 可训练参数 / `parameters` | 实际参与学习且 `requires_grad=True` 的参数元素个数 | E1 COCO 为 3,542,567 对 3,510,624，仅匹配下游可训练参数；E4 已采用完整系统总参数口径，须另列冻结/可训练参数 |
...
完整证据位于外部工作区 `reports/E1-{A,B,C,S}/`、`jobs/` 和 `E1-summary.json`；详细归档见 [E0/E1 执行记录](https://github.com/Frank95zz/YOLO-Master/blob/373b8a4a563bd885c13b78652a75ae4dee7edfdd/experiments/d1/E0_E1.md)，不把 checkpoint 或完整预测放入 Git。
...
正式基线已按第 4.3、9 节确定为总参数匹配的 YOLO26-L width0.9375；不能沿用旧同参声明或把旧参照成绩当作新架构的正式匹配结果。
...
## 9. E4 P1 两个数据集的正式对照（总参数匹配基线）

**2026-09-11 用户确认按完整系统总参数量匹配。** 冻结侧包含 DINOv3 ViT-S/16 和 BN64 下游；Scratch 采用标准 YOLO26-L 拓扑、depth=1.0、width=0.9375、max_channels=512，配置为 `ultralytics/cfg/models/26/yolo26-d1-scratch-total-l.yaml`。两个数据集共用骨干与 Neck，仅切换 nc。选择只依据参数审计与结构合法性，不依据 AP；1% 是本项目工程容差，不冒充课题官方定义。详细依据见 [总参数基线说明](SCRATCH_TOTAL_PARAMETER_BASELINE.md)。

ViT-S 为 21,596,544 参数；VisDrone 冻结总参数为 22,936,803，Scratch 为 23,032,340（+0.4165%）；COCO 冻结总参数为 23,001,383，Scratch 为 23,133,560（+0.5746%）。旧 E1 S 仅为历史对照，不改写其匹配口径。

**启动门槛仍保留：** AUX_STAR、完整配对训练合同、实际显存/吞吐、训练与恢复验收以及运行预算尚需锁定；本次只确定架构，不授权启动 E4。下面的种子和训练上限保留为预算草案。
正式对比前完成以下确认清单：

1. 锁定最终冻结模型的 Teacher ID/权重、Adapter、融合/LatentMixture、Neck（如有）和 Detect，记录结构配置、代码及 SHA256。
2. 本项目按用户 2026-09-11 确认的完整系统总参数口径执行；分别列出冻结/可训练参数和实际匹配误差。课题负责人对验收口径若有补充，单独记录，不将用户确认或工程容差写成导师已确认的定义。
...
| --- | --- | --- | --- | --- | --- |
| COCO 2017 | ViT-S + BN64，固定融合与待锁定的 AUX_STAR | YOLO26-L width0.9375，nc=80，23,133,560 参数，随机初始化 | 0、1、2 | 暂拟各 100 epochs | 暂拟 6 |
| VisDrone2019-DET | 同一最终架构与 AUX_STAR，适配 10 类 | 同一 Scratch 骨干/Neck，nc=10，23,032,340 参数，随机初始化 | 0、1、2 | 暂拟各 300 epochs | 暂拟 6 |
...
| 项目 | 完成标准 |
| --- | --- |
| P1 基线确认 | 2026-09-11 用户确定总参数口径，完整系统含 Teacher；YOLO26-L width0.9375 已完成两数据集参数/前向审计，完整训练门禁与预算仍需通过 |
