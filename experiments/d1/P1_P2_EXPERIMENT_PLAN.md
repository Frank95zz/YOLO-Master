...
**状态（2026-09-08 补充时点）：E0 工程门槛已通过；E1 的 A/B/C 已完成 50 轮和最终独立评测，S 仍在训练，四组汇总结论待 S 完成；E2 及以后仍待实施。**
实际实现、验收、运行入口及结果边界见 [E0/E1 执行记录](E0_E1.md)。这不代表已经达到 P1/P2 收益目标。

本次根据 E1 阶段结果补充第 3.1、6.2-6.4、8.3、11.4 和 12.2 节。保留既定的固定轮数、评测、参数匹配、多 seed 和成本判据；不修改正在执行的 S 组合同，也不以更新文档授权启动后续训练。
...
1. **先完成 S 并关闭 E1 证据链。** 不提前停止或改变其配方；四组统一按第 50 轮标准 AP、实际更新数及成本比较，标准 best 仅作补充。
2. **后续以 weighted_sum 为主方向。** A 保留为机制对照，不再默认增加 router_only 的大规模训练；B/C 均保留为 aux 候选，不将 seed0 的小幅 AP 差定为正式胜负。
3. **先完成 E2，再分批开展 E3。** VisDrone 的协议、NVMe 数据/缓存、小样本闭环通过后，完成预注册 aux 扫描和复核，锁定 AUX_STAR。
4. **满足门禁后进入 E4。** 形成两数据集、三 seed 的正式配对结果；如果触发第 6 节的 80% 暂停线，先执行第 8 节的解除条件，不自动展开 COCO 长训练。
...
2026-09-08 快照：以下来自运行批次 `e01-20260908/attempt-04`，seed0，完整 COCO val2017 的 5,000 张图片。
...
| S | 参数匹配 Scratch / 不适用 | 待第 50 轮完成 | 待完整窗口汇总 | 待定 |

A/B/C 均完成 50 轮、15,450 次有效 optimizer 更新；最终 last 与 standard-best 均通过严格重载及全量评测，checkpoint 中 Teacher 参数数目为 0。
证据位于外部工作区的 `reports/E1-{A,B,C}/training-result.json`、`final/last/report.json`、`final/standard-best/report.json` 和 `cost.json`；详细归档由 [E0/E1 执行记录](E0_E1.md)维护，不把 checkpoint 或完整预测放入 Git。

本批训练/评测的实际代码提交记录包含 `9347a8b9541790c980d5371ae9cf79630ecc606d` 与 `d9151b49188651b9c3562fc6b7877a5b3f1626e9`，二者之间为文档变更，相关训练源码指纹相同。
共同实际源码 SHA256 为 `f4d263706445eb649ba845995e0bb9e9edaa3e0372eb8af8ecdfe6b591733d09`，合同 SHA256 为 `619bc71e155f3fe291179189edc656f6a8399b50f7bf911f123fc7b5e18d0322`。
...
- B 比 C 仅高 **0.2149 个 AP 点**，属于预设的筛选并列区间。B 可暂作默认 aux 的机制参照，C 继续作为关闭 aux 的候选；这不是宣布 B 已成为 AUX_STAR。
- B/C 的 standard-best 都在第 30 轮，至第 50 轮分别下降约 **0.4788 / 0.3992 个 AP 点**。这是固定采样点上的曲线现象，不足以单独证明过拟合；不能据此把正式预算改为 30 轮，或用 best 替代第 50 轮主结果。
...
保持 S 的初始化、100 轮调度前缀、50 轮窗口、batch384 和标准评测频率不变。
完成后检查四组固定末轮结果、15,450 次更新、最终重载/哈希与全部 5,000 张验证图，再生成配对摘要。
主要回答三件事：固定轮数精度保留率、相同 epoch 的收敛速度、达到共同 AP 的实际或潜在成本。

以 B 的已测末轮 AP 为例，定义 `T90 = 28.8345947733 / 0.90`、`T80 = 28.8345947733 / 0.80`：

| S 第 50 轮标准 AP（百分制） | B 的对应保留率 | 下一步 |
| --- | --- | --- |
| 不超过 T90，约 32.04 | 至少 90% | 支持继续精度/成本验证；尚未证明节省 50% |
| 高于 T90、不超过 T80，约 36.04 | 80% 至不足 90% | 先完成 aux 和误差分析，再决定是否需要结构筛选 |
| 高于 T80 | 低于 80% | 按原门禁暂停整套 COCO 多 seed 长训练，执行 E3 的解除条件 |

此表是 B 的示例换算；实际门禁使用候选中最高的固定第 50 轮 Frozen AP，按未舍入数值计算，不用 best AP 或表中两位小数判边界。
筛选不是 P1 验收：E4 仍需两数据集完整预算和 seed0/1/2，正式精度保留与成本结论按第 11 节形成。
S 未完成前只记录同轮早期优势，不外推其第 50/100 轮精度，不提前宣称 Frozen 胜出。
...
如果采用新结构，则新建实验身份并重新核对两数据集可训练参数；超出 1% 时先重新匹配 Scratch，不能沿用旧同参声明或旧对照成绩。
...
### 8.3 基于 B/C 结果的分批实施重点

先通过 VisDrone 数据/官方评测对齐和真实 batch 工程门槛，再按第 8.2 节的既定矩阵分批执行；最多 16 次运行的上限不变，不因增加本节而自动启动。
第一批完成 9 组系数网格，可优先运行默认与全零系数；全零系数不是 gain0 实验，不将两条数值路径擅自合并。
第二批在入选非零系数上完成 gain 扫描，包含关闭 aux 的 gain0；第三批执行候选/默认配置的独立 seed 复核。
分批是执行顺序，不得看过前几组结果后隐去其余预注册候选或负结果；缩减矩阵需事先修订并说明。

重点验证“raw balance/z 降低是否转化为检测收益”，同时记录归一化/预算限制后的 effective_aux 和相对检测梯度。
不能用 B 比 C 高 0.21 AP 直接选定系数，也不能从 B/C 的受干扰总时长推断关闭 aux 更快。
如果关闭 aux 最终更优或效果无法稳定区分，应如实报告；不为证明 aux 有用而追加未登记的大规模试验。
AUX_STAR 锁定后再进入正式两数据集配对训练，不按各数据集的结果临时分别挑选最佳 aux。
...
目前正常无外部 CPU 争用时，Frozen 的单轮训练加内置验证约 144-145 秒，S 的近期单轮约 166 秒。
以 144.5/166.4 估算，同轮耗时下降约 **13.2%**，只是局部吞吐参考，未包含缓存抽取和全部独立评测，不是端到端 GPU-hours 已下降 50%。
同样，吞吐提升倍数与耗时下降比例不是同一个数，不能混用。

早期收敛是更值得验证的成本来源：第 15 轮独立标准 AP，B 为 **27.5217**，S 为 **20.6465**。
这只支持同轮早期优势，不预告 S 的最终精度；待正式 E4 三 seed 结果形成后，仍使用第 11.3 节定义 AP_target，不将 E1 临时精度指定为新的正式目标。
...
**2026-09-08 E1 实测补充：** B/C 的 `cost.json` 训练作业时间分别为 8,551.05 / 7,842.26 秒，约 2 小时 22 分 31 秒 / 2 小时 10 分 42 秒，对应 14.2518 / 13.0704 GPU-hours（6 卡）。
其范围是记录到的训练作业，含作业内等待与定期评测，不含缓存抽取、停机间隔和作业后的最终独立评测；不是冷启动总成本。
两组约 11 分 49 秒的差异受 B 的外部 CPU 争用影响，不能用作 aux 开销结论。S 尚未完成，整套 E1 的最终时间以全部报告汇总为准。
后续 COCO 排期优先参考第 11.4 节的正常单轮实测，并单列加载、定期标准评测、最终校验等开销；不能把该快照外推为 VisDrone 或新结构的速度保证。
