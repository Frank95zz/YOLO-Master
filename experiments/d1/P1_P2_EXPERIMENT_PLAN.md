...
**状态（2026-09-09）：E0 工程门槛已通过；E1 的 A/B/C/S 均已完成 50 轮、15,450 次有效更新以及 last/standard-best 的完整 5,000 图独立评测；E2 数据、完整缓存、10 类训练与续跑、短基准和实际预测的官方评分均已通过；E3 及以后未启动。**
...
**2026-09-09 E3 方案修订：aux 消融仅在 VisDrone 开展，不新增 COCO aux 消融或开关复核。** balance/z 的 9 个组合各跑 seed 0、1、2、3，再对选中的非零组合比较 4 个 gain，每个 gain 同样跑四个 seed；复用已有的 gain0.1 四次结果后，共 48 个独立有效运行，每次仍为 300 轮调度的前 60 轮。本次只修改方案，不授权启动训练。
...
| P2：扫描 latent aux 权重 | 仅 VisDrone：balance/z 的 9 组和随后 4 个 gain 均覆盖四个 seed；按均值选参 | 全部候选四 seed 明细、均值/标准差、负结果、路由/梯度及成本表 |
...
- E3 的每个候选使用 **seed 0、1、2、3**；E4/E5 正式对照仍使用 **seed 0、1、2**。为避开 E3/E4 已用种子，E6 成本停止确认改用独立 seed 4、5、6，确认运行数不变。
...
| E3 | 仅 VisDrone：9 组 balance/z 各四 seed，再比较 4 个 gain 各四 seed | 36 + 12 = 48 个独立运行，按四 seed 平均 AP 锁定 aux；不新增 COCO aux 消融 |
...
3. **先完成 E2，再分批开展 E3。** VisDrone 的协议、NVMe 数据/缓存、小样本闭环通过后，完成每个候选四 seed 的两阶段 aux 扫描，锁定 AUX_STAR；不在 COCO 新增 aux 扫描或开关复核。
...
| 均值、样本标准差 | 按各阶段固定 seed 集计算 AP、成本、配对差和 saving；E3 为 seed0/1/2/3，E4/E5 为 seed0/1/2；`s=sqrt(sum((x-mean)^2)/(n-1))` | 均值描述典型结果，标准差描述跨训练种子波动，不是同一训练的 epoch 波动 |
| 配对差及95% t区间 | 先按相同 seed 得到差值/比例，再算 `mean ± t(0.975,n-1)*s/sqrt(n)`；E4/E5 的 n=3 时 t≈4.303，E3 的 n=4 时 t≈3.182 | 小样本区间不稳定；E3 使用同一批结果选参，区间只作探索性描述，不作为独立确认或多重比较校正后的显著性结论 |
...
## 8. E3 P2 辅助损失扫描（仅 VisDrone，全部候选四 seed）
...
固定 ViT-S、weighted_sum、batch96、`mixture_aux_budget=3.0` 和 300 轮学习率调度；
每次执行前 60 轮，不把余弦周期压缩为 60 轮，不因曲线提前见顶而改变筛选时点。
所有候选均使用 **seed 0、1、2、3**，不再采用“seed0 预筛，再只给少数候选补种子”的流程。
...
| --- | --- | --- | --- | ---: | ---: |
| 第一阶段：balance/z 网格 | gain=0.1，budget=3.0 | balance ∈ {0,0.01,0.1}；z ∈ {0,0.001,0.01}，完整 9 组笛卡尔积 | 0、1、2、3 | 9 × 4 = 36 | 36 |
| 第二阶段：全局 gain | 第一阶段选中的非零系数组合；budget=3.0 | gain ∈ {0,0.03,0.1,0.3} | 0、1、2、3 | 4 × 4 = 16 | 12；gain0.1 的四 seed 结果复用第一阶段 |

总计 **48 个独立有效训练运行**，即 36 + 16 - 4 = 48；每次 60 轮，总计 2,880 个训练 epoch。
两张阶段表可引用同一组 gain0.1 结果，但不得把它们重复计为新训练或重复累计成本。
不再另设原方案的“默认/候选 seed1、seed2 复核”第三阶段；四个 seed 已覆盖每个候选。
第一阶段全部 36 次有效结果齐全后才能选 balance/z；第二阶段需四个 gain 各四 seed 结果齐全后才能锁定 AUX_STAR。

第一阶段同时保留全零、仅 balance、仅 z 和两项同时开启的对照。
第二阶段选择第一阶段四 seed 平均 AP 最高的**非全零**系数组合；若全零组领先，仍如实报告，
...
### 8.3 四 seed 选参规则与统计边界

主指标为 VisDrone 官方评测口径的**固定第 60 轮 AP**，按 seed0/1/2/3 的算术均值排名；
保存原始 0..1 数值后计算，展示时才换算 AP 点，不用显示舍入值决定名次。
standard-best 和其他 epoch 的最高值只作补充，不替代固定第 60 轮。
每个配置报告四个原始 AP、均值、样本标准差（分母 n-1，n=4）、AP50/AP75、AR 与实际 GPU-hours。

第一阶段按四 seed 平均 AP 选出一个非零 balance/z 组合；第二阶段同样按四 seed 平均 AP 选 gain。
...
报告配对差的均值、样本标准差与正/负差的 seed 数；若给出 95% t 区间，n=4、自由度 3，且注明小样本局限。
四个 seed 都参与了配置选择，相关区间仅为探索性描述，不能当作独立确认或已校正多重比较的显著性证据。
...
第一批是 9 组各四 seed，共 36 次；可以优先执行默认与全零配置，但不得只看 seed0 就淘汰其他候选。
第一批完成后报告四 seed 汇总和选中的组合，第二批新增 3 个 gain 各四 seed，共 12 次；结果复用按第 8.2 节校验。
每批开始前报告实测 ETA、存储和资源预算并获得明确启动确认；本次文档修订不启动任何实验。
...
原 E1 80% 暂停门禁仍保留，但当前 E1 未触发；以下仅为该门禁触发后的独立系统确认，不是 COCO aux 消融，也不包含在 E3 的 48 次中：
...
冻结 AP_target、配方和停止规则后，在该数据集启动独立 seed4/5/6 的 Frozen/Scratch 配对确认（避开 E3 已参与选参的 seed0/1/2/3，运行数不变）：
...
| E3 VisDrone aux（2026-09-09 修订） | 36 + 12 = 48 次，各 60 轮，共 2,880 epochs | 两阶段全部候选四 seed；待完整 epoch 基准，ETA 含加载、定期/最终评测，不照搬 COCO 每轮时间 |
...
| P2 aux 研究 | 仅 VisDrone：9 组 balance/z 各四 seed，随后 4 个 gain 各四 seed，复用后 48 次；固定第 60 轮均值/标准差、gain0 配对差与路由机制分析齐全，包含负结果；不声称完成 COCO aux 独立贡献验证 |
