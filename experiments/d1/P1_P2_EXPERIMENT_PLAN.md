...
**状态：方案已记录，后续代码与实验待实施。本文件不代表已启动训练，也不代表已经达到收益目标。**
...
**本次文档提交不实现以上改动。所有新接口在实现、测试和代码提交完成前均不可作为现成命令执行。**
...
| E1 | 完整 COCO 的 A/B/C + Scratch，seed0，100 轮调度前 30 轮 | 判断空间融合及 aux 开关作用，选择固定融合方式 |
...
完整 train2017/val2017，不重建现有 ViT-S 缓存；seed0，统一运行 100 轮调度的前 30 轮。

| ID | 模型/融合 | balance / z | latent_aux_gain | 唯一主要对照 |
| --- | --- | --- | --- | --- |
| COCO-A30 | ViT-S，router_only | 0.01 / 0.001 | 0.1 | 新代码基线 |
| COCO-B30 | ViT-S，weighted_sum，权重 [1,1,1] 归一化 | 0.01 / 0.001 | 0.1 | 与 A 比较空间融合 |
| COCO-C30 | 同 B | 0.01 / 0.001 | 0 | 与 B 比较 aux 开关 |
| COCO-S30 | 参数匹配 Scratch | 不适用 | 0 | 同预算从零训练参照 |

四组均新建运行。旧 A100 或旧 Scratch30 不直接混入此表。
固定推理语义：LatentMixture 训练和评测都使用全部 4 个专家，不启用 top-k 剪枝/稀疏回退。

筛选依据为第 30 轮独立 COCO AP；每 5 轮保留曲线。出现 NaN、数据错位、丢样本、静默跳更新则工程失败，
...
最多增加一次采用固定融合和 `AUX_STAR` 的 COCO30、seed0 确认，
与 E1 Scratch30 比较；仅当代码、初始化规则、数据和预算身份仍完全一致时复用该 Scratch 参照。
...
| 工作 | 数量与预算 | 当前排期参考 |
| --- | --- | --- |
| E1 COCO 筛选 | 3 Frozen + 1 Scratch，各 30 轮 | 约 4.95 小时，须用新短基准更新 |
...
仅 COCO 两阶段的粗略排期已约 30.75 小时，不能承诺整个 P1/P2 一天完成。
若触发 E3 后的 COCO30 门禁确认，额外 Frozen 运行按旧速度约 1.18 小时；
